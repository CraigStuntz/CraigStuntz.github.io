...
* Chapter 3: Breadboarding the VCO
...
in. But it makes sense to start with a VCO, so that the other modules
will have some sound to modify when built, and the sawtooth/PWM VCO is
the most generally useful oscillator in the series, so I started with that kit.
...
at the emitter within the 0-550mV range what is appropriate for the NPN 
...
is around 0-6V, and you really don't want any of that at the far extremes of the 
potentiometer.
