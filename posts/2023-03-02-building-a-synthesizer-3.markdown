---
title: "Building a Synthesizer, Chapter 3: Breadboarding the VCO"
series: Building a Synthesizer
chapter: "3"
chapterName: Breadboarding the VCO
...
oscillator in the series, so I started with that kit.

## On Making Mistakes

I'm going to describe the whole build process, with a particular focus on the 
...
The quote above goes both ways. Debugging teaches you more about writing code 
...
The heart of the kit, in my opinion, is breadboarding. This is what distinguishes
...
with breadboarding you build _and evolve_ the
circuit for the VCO, adding features as you go. 

The breadboard constuction proceeds in several phases.
...
<figcaption>It's... oscillating!</figcaption>
</figure>

Building the oscillator core on the breadboard went easily, and the circuit 
worked the first time I tried it. This part "just worked."

Note that the wave you can see in the image is sometimes called an "inverted" 
...
as an [LFO](https://en.wikipedia.org/wiki/Low-frequency_oscillation).

What frequency is the oscillator producing? Well, my oscilloscope tells me that
...
One thing which I will not do in this series is to explain _how_ the circuits 
work. For that you can [read the very fine manual](https://www.ericasynths.lv/media/VCO_MANUAL_v2.pdf), 
...
nothing, no knowledge assumed" explanations I've seen of audio circuitry.

Having said that, I do think it's worth sharing 
...
distinguishing red and green is challenging for you.)

<div style="clear: both;"></div>

#### Add Output Buffer

So now we have an oscillation, we're done with the "VCO," right? Not by a long
...
of the headphones, etc., will change the frequency of vibration.

The circuit is quite simple, but I encountered a few issues building it.
...
<figcaption>What is this component?</figcaption>
</figure>

One minor frustration I had with the instructions for this kit concerned the 
...
<figcaption>Actual diagram from instructions</figcaption>
</figure>

A much bigger problem was that when I powered up the circuit the TL074 op amp 
...
[It turns out I'm not the first to make this mistake!](https://modwiggler.com/forum/viewtopic.php?t=257249&start=75)
...
<figcaption>Suggested/corrected diagram</figcaption>
</figure>

Worse, the TL074 is pictured incorrectly in the instructions! The text is 
...
_Also,_ a further warning, just because the instructions seem to be going out of 
their way to confuse you on this point: The orientation of the two ICs is 
_different_ in the breadboard layout vs. the PC board layout. All I can say is 
...
same, and getting it wrong means a cooked op-amp chip.
...
So I "borrowed" another TL074 from a different kit installed the chip correctly,
...
> Plug some cheap headphones into the audio jack socket, and you should be able to
> hear a very high-pitched tone.

When I plugged my headphones into the jack I heard... mostly a low-frequency 
...
wire fixed the click, and I could hear the correct oscillation. 

Let me spell this out with schematics, since 
[it seems like others have had this problem](https://modwiggler.com/forum/viewtopic.php?p=3696054#p3696054):
...
<figcaption>This causes 5-10 Hz clicking</figcaption>
</figure>
...
<figcaption>Correctly wired circuit</figcaption>
</figure>

<div style="clear: both;"></div>

The schematics make my mistake super-clear! Whereas if you look at the actual 
...
After working through all of these problems, I think that these mistakes, which 
...
chips onto a board and then you're done. It's wonderful!

(Note that the audio output is somewhat loud. If your headphones don't have a volume 
control on them, I'd recommend keeping them off of your ears, at least at first.)

#### Digression: An Optimization?

The VCO at this point has essentially three sub-circuits, identifed below:
...
<figcaption>Sub-circuits</figcaption>
</figure>

The oscillator core emits a sawtooth wave ranging from about 1.7 to 7.2V. This 
...
voltage range being more centered in the op amp's range. 

Since I find it easier to think in schematics than in board layout, I first 
...
<figcaption>This does not work</figcaption>
</figure>

Instead of being two distinct sub-circuits separated by the buffer, the 
...
#### Changing the Frequency with a Potentiometer

Next, we experiment with changing the frequency of oscillation by substituting 
...
potentiometer will eventually become the tuning knob. 

The only real problem in this phase of the build was mounting the potentiometer 
to the breadboard; the pot I was given in the kit was much bigger than the pot 
drawn on the assembly diagram. I found a way to make it work (see below).
...
<figcaption>VCO at the end of this stage</figcaption>
</figure>

I find it helpful to look at a schematic, but there is no (complete) schematic
for this stage in the directions, so here's one I made:
...
#### Changing the Frequency with a Voltage Control Input

Here we just add a jack and use a passive mixing circuit -- nothing more than 
a couple of resistors -- so that we can control the pitch with _both_ an 
...
a 4 or 14k resistor to bring the sum down a bit. 
...
#### Deal with Component Temperature Variations

Having added a Control Voltage input in the previous step, we now must rip it 
out to add support for additonal components. 

Because the NPN transistor warming lowers its resistance to ground, raising the
...
I really needed the schematic here:
...
One thing to point out here is that the PNP transistor's collector is connected
...
transistor's collector to -12V instead of ground does that. 

#### Add Tuning

Now we add the tuning knob and CV input back into the circuit, also adding a 
...
There's a trim potentiometer which is connected to a 1.5kΩ resistor in 
...
oscillator's fine tuning adjustment. 

The biggest problem I had with building this part of the circuit was that I 
...
which bordered on highway robbery, but _I don't care,_ because I can't tell you
how nice it is to have an electronics parts store within a few blocks of my 
house and I would very much like them to remain in business. 

The coarse tuning potentiometer has returned, now
...
the tuning potentiometer. How low? 

The tuning potentiometer forms part of a voltage divider between the +12V and 
-12V supplies. The voltage divider output is:

| Potentiometer setting | Voltage divider calculation       | Output |
| --------------------- | --------------------------------- | ------ |
| 0%                    | 24V * 100kΩ / (100kΩ+133kΩ) - 12V | -1.7V  |
| 50%                   | 24V * 150kΩ / (150kΩ+83kΩ) - 12V  |  3.4V  |
| 100%                  | 24V * 200kΩ / (200kΩ+33kΩ) - 12V  |  8V    |

Compare that to the circuit in the "Changing the Frequency with a Voltage 
Control Input" section:

| Potentiometer setting | Voltage divider calculation | Output |
| --------------------- | --------------------------- | ------ |
| 0%                    | 12V * 0kΩ / (0kΩ+100kΩ)     |  0V    |
| 50%                   | 12V * 50kΩ / (50+50kΩ)      |  6V    |
| 100%                  | 12V * 100kΩ / (100kΩ+0kΩ)   |  12V   |

So we can see that the voltage is indeed lower. More importantly, the range of
the coarse tuning adjustment is now closer to a "musically useful" range, which 
is around 0-8V.

#### Add PWM Wave Shape

Other than the breadboard getting a bit crowded and the potentiometers not 
really fitting into the breadboard well as usual, this step went fairly easily.
...
going to the `-` input of the op amp.

I do find the explanation of using an op amp as a comparitor in the directions
a bit confusing, however. I much prefer this version, from _Make: Analog Synthesizers,_ 
by Ray Wilson:

> Here we are applying a... voltage to the inverting input [`-`] of 
> the op amp, and the noninverting input [`+`] is the input to the comparator. If we 
...
In short, think of the op amp comparitor as a buffer with the feedback resistor 
set to infinity instead of nothing.

#### Add PWM Width Control

This is just a potentiometer voltage divider going into the inverting [`-`] 
input of the comparitor op amp above. 

This pretty much just worked, although once again getting the potentiometer to 
...
purchased a larger breadboard for the remaining kits. 

#### Add Output Level Control

Here we add a couple of op amps to normalize both the sawtooth and the square 
wave outputs. Finally we can hear the square wave as the output is no longer so
hot that it would damage your headphones if you connect it. 

Here's my final physical circuit:
...
oscillator by pressing the two "Play Audio" buttons.
...
<figcaption>Simulated VCO</figcaption>
</figure>
...
* [DIY VCO Part 1: The analog oscillator core anyone can build](https://www.youtube.com/watch?v=QBatvo8bCa4),
  [DIY VCO Part 2: The simplest exponential converter](https://www.youtube.com/watch?v=4qxgwN9aq8E),
  [DIY VCO Part 3: Tuning your VCO (and making sure it stays in tune)](https://www.youtube.com/watch?v=dd1dws6pSNo),
  and
  [DIY VCO Part 4: Shaping Sawtooth to Square](https://www.youtube.com/watch?v=E76z9ZTc5po). 
  Four-video series by Moritz Klein, 1:02:00 all together. It's important to 
  note that in this series he builds a _somewhat different_ VCO than the one 
  which ships in the mks x es.EDU kit. But it has a lot in common and I still 
  think this video series is very, very useful.